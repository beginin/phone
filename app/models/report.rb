    ...
    @columns ||= []
 	...
 	def self.rep
 		sql = "WITH simnumlogs_full AS (
			...
			tnumbers_full AS (
			SELECT tnumbers.* , tarifs.name
			from tnumbers 
			LEFT JOIN tarifs  ON (tnumbers.tarif_id = tarifs.id)
			),
			upr AS (
			SELECT colllogs.tnumber, tnumbers_full.id as tnumberid ,userlogs_full.userlogid ,SUM(colllogs.coast)
			FROM colllogs 
			...
			GROUP BY colllogs.tnumber ,tnumberid , userlogid 
			),
			fin AS (
			...
			SELECT upr.tnumber,upr.sum as uprsum,fin.sum as finsum , userlogs_full.secondname,userlogs_full.firstname,userlogs_full.midlename,
